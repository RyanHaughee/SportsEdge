...
con <- get_db()
db_table <- "nfl_kick_stats"

# No scientific notation
options(scipen = 9999)

# Kicking box scores from selected season
nfl_kick_stats <- load_player_stats(seasons = 2023,
                                     stat_type = "kicking"
)
...
non_numeric_cols <- names(nfl_kick_stats)[!sapply(nfl_kick_stats, is.numeric)]
