...
con <- get_db()
db_table <- "nfl_off_stats"

# No scientific notation
options(scipen = 9999)

# Offensive box scores from selected season
nfl_off_stats <- load_player_stats(
  seasons = 2023,
  stat_type = "offense"
)
...
non_numeric_cols <- names(nfl_off_stats)[!sapply(nfl_off_stats, is.numeric)]
